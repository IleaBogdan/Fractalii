...
namespace Fractalii.TreeFractal
{
    public class treeFractal
    {
        ...
        // recursive function
        public void Generate_fractal1(double size, PictureBox p, int start_x, int start_y, double angle, int maxLevel)
        {
            ...
            if (maxLevel>0)
            {
                ...
                Thread.Sleep(2);
                Generate_fractal1(size * 0.90, p, end_x, end_y, angleL, maxLevel- 1);
                Generate_fractal1(size * 0.70, p, end_x, end_y, angleR, maxLevel- 1);
            }
        }
        public void Generate_fractal2(double size, PictureBox p, int start_x, int start_y, double angle, int maxLevel)
        {
            double angleL, angleR;
            int end_x, end_y;
            (end_x, end_y, angleL, angleR) = calculation(start_x, start_y, size, angle, start_angle_right, start_angle_left);
            ...
            {
                QItem = queue.Dequeue();
                draw(QItem.start_x, QItem.start_y, QItem.end_x, QItem.end_y, p);
